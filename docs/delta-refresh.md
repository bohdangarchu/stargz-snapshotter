...
The swap itself goes through `readerRef`, an RWMutex-wrapped `reader.Reader` that the `layer` and the FUSE `fs` struct share. Every FUSE metadata/open path takes a read lock via `rr.get()`; `RefreshBlob` takes the write lock via `rr.swap()`, so the new reader becomes visible to all nodes at once and an in-flight read that already holds the old reader finishes against it. The on-disk cache format is unchanged.
