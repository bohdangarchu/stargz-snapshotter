...
3. **Scope background fetch.** When `--with-background-fetch` is set, fetch only the changed/added chunk ranges, not the whole new blob.
...
This matches the target use case: model-weight updates rebuild the weights layer and leave the base alone.
...
The TOC diff compares chunk digests (file content) and entry structure (parent id + name at the same metadata id). It does **not** compare per-entry mode, uid/gid, mtime, xattrs, symlink target, or file type. If a rebuild changes only those and leaves the bytes identical, FUSE keeps serving the old `stat` / `readlink` / `getxattr` results until the layer is unmounted.

Acceptable for the target use case (model-weight rebuilds change content alongside any metadata change). Two costs avoided:

- **Always-fallback semantics.** Hashing every attr and tripping whole-layer fallback on any drift collapses to "always fall back" if the producer doesn't pin mtimes — common enough to make the granular path useless.
- **Granular attr invalidation.** Doing it correctly means a separate FUSE attr-cache invalidation path alongside `NotifyContent`. Worth building only if a real workload reports stale metadata.

If needed later, the fix is additive: extend `entrySignature` with an attr fingerprint, surface mismatches as a new `MetadataChanges []uint32` on `DeltaResult`, have the consumer issue per-inode attr invalidation.
